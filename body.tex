[TODO: Add a convention that sets the based field to be $\mathbb{C}$, every tensor category to be finite, semisimple, pivotal, and every module category to be finite and semisimple. Remove conditions in statements.]
...
\begin{definition} (Balanced Module Category Functor)
  ...
  category, $_{C}L_{E}$ be a $C{\text -}E$ bimodule category.

  \noindent Then a $D$-balanced $C{\text -}E$ bimodule category functor (from
  $M \times N$ to $L$) is a pair
  ...
  $C{\text -}E$ bimodule category functor and $\alpha$ is a natural isomorphism between the
...
\begin{definition} (Category of Balanced Functors)
  ...
  objects are $D$-balanced $C{\text -}E$ bimodule category functors $M \times N \to L$
...
\begin{definition} (Balanced Tensor Product)
  ...
  \noindent Let $C, D, E$ be tensor categories. Let $_{C}M_{D}$ be a $C{\text -}D$
  bimodule category, and $_{D}N_{E}$ be a $D{\text -}E$ bimodule category.

  \noindent Then the balanced tensor product $_{C}(M \boxtimes_{D} N)_{E}$ is
  the $C{\text -}E$ bimodule category defined as the initial $D$-balanced module
  functor from $M \times N$; i.e. for each map from $M \times N$ to some $C{\text -}E$
  bimodule category $L$, there exists a unique $C{\text -}E$ bimodule category functor
...
\begin{definition}\label{definition/skein-category} (Skein Category)
  ...
  be finite, semisimple bimodule categories. One can in a similar way define
  the $C_{0}{\text -}C_{n}$ bimodule category
  \[
...
\begin{definition} \label{definition/induced-functor-on-skein-category} (Induced Functor on Skein Category)
  ...
  $C_{i{\text -}1}{\text -}C_{i}$ bimodule category functor. Then it naturally
  induces a linear functor between the skein categories
  \[
...
provide another direct proof for this equivalence in the appendix
\ref{theorem/degenerated-main-theorem}.
...
\section{Proof of the Main Theorem}\label{section/proof-of-equivalence}
...
\begin{lemma} (Images of skeins) \label{lemma/image-of-skein}
  ...
  and $(F,\alpha) \in Fun^{C{\text -}bal}(M \times N, L)$. For each morphism in
  $sk(M,C,N)$ of the form $\III{m'}{n'}{m}{n}{\phi}{\psi}{c}{\pi}{c'}$, define
  the image of it under $\tilde{F}$ to be the composed morphism
  \[
    F(m,n)
...
\begin{proof}
  Let $(F, \alpha) \in Fun^{C{\text -}bal}(M \times N, L)$. It suffices to construct $G \in Fun(sk(M,C,N), L)$ such $\Theta(G) \simeq (F,\alpha)$.
...
\begin{theorem} (Main Theorem: Skein Construction of Balanced Tensor Product) \label{theorem/main-theorem}
  ...
  {}_{C_{n{\text -}1}}M^{n}_{C_{n}}, \,
  $ be finite, semisimple bimodule categories.
  Then we
  have an equivalence of $C_{0}{\text -}C_{n}$ bimodule categories.
  \[
...
\begin{theorem} \label{theorem/degenerated-main-theorem} Let $C$ be a finite,
  semisimple, pivotal tensor category. Let $M_{C}$ be a right $C$-module
  category. Then we have an equivalence of categories $sk(M,C,C) \simeq M$.
\end{theorem}
